...
<html lang="zh-CN">
  	<head>
		...
		<link href="assets/pages/css/userHeadPicture.css" rel="stylesheet">
	</head>
	<body>
				...
				<div class="section-botton">
					...
					<div class="section-botton-right">
						...
						<div id="currentLocation">
							<p><a href="dataManagement">用户数据</a><span>&#155;</span>更换图片</p>
						</div>
						<input type="hidden" id="errorInfo" value="${requestScope.msg}"/>
						
						<c:if test="${requestScope.pageInfo.total >0}" var="notNull">
							<ul class="ulImgClass">
								<c:forEach var="userHeadPicture" items="${requestScope.pageInfo.list}">
									<li class="li">
										<h4></h4>
										<div class="position-relative">
											<div>
												<c:choose>
													<c:when test="${userHeadPicture.headPicture != ''}"><img src="${pageContext.request.contextPath}/assets/pages/img/headPicture/${userHeadPicture.headPicture}" class="imgClass" /></c:when>
													<c:otherwise></c:otherwise>
												</c:choose>
											</div>
											<input data-id="${userHeadPicture.id}" accept="image/*" type="file" class="fileUpload" />
										</div>
										<div class="divClass">
											<button data-id="${userHeadPicture.id}" type="button" class="btn btn-info update">
												<span class="glyphicon glyphicon-pencil"></span> 修 改
											</button>
											<button data-id="${userHeadPicture.id}" type="button" class="btn btn-danger delete">
												<span class="glyphicon glyphicon-trash"></span> 删 除
											</button>
											<button type="button" class="btn btn-primary save">
												<span class="glyphicon glyphicon-ok"></span> 保 存
											</button>
											<button data-id="${userHeadPicture.id}" type="button" class="btn btn-default cancel">
						...
						<p class="text-right">
							<button type="button" id="add" class="btn btn-info addButton">
								<span class="glyphicon glyphicon-plus"></span> 添加更换图片
							</button>
						</p>
						...
						<c:if test="${notNull}">
							<div id="pageSizeAndpageNum">
								<ul>
									...
									<c:forEach var="pagingButton" items="${requestScope.userHeadPicture_pagingButton}">
		...
		<script src="assets/pages/scripts/userHeadPicture.js"></script>
	</body>
</html>
