...
<html lang="zh-CN">
  	<head>
		...
		<link href="assets/pages/css/gameInformationTheme.css" rel="stylesheet">
	</head>
	<body>
				...
				<div class="section-botton">
					...
					<div class="section-botton-right">
						...
						<div id="currentLocation">
							<p><a href="dataManagement">游戏数据</a><span>&#155;</span>游戏资讯</p>
						</div>
						<input type="hidden" id="msg" value="${requestScope.msg}" />
						
						<div>
							<p class="findClass">
								<input type="text" autocomplete="off" id="theme" value="${sessionScope.gameInformationTheme_theme}" placeholder="搜索主题" maxlength="10" /><img id="select" alt="搜索" src="${pageContext.request.contextPath}/assets/pages/img/common/ImageSelect01.png" />
							</p>
						</div>
						
						<c:if test="${requestScope.pageInfo.total >0}" var="notNull">
							<c:forEach var="gameInformationTheme" items="${requestScope.pageInfo.list}">
								<dl class="dlClass text-left">
									<dt class="text-center"><img class="imgClass" src="${pageContext.request.contextPath}/assets/pages/img/gameInformationImg/${gameInformationTheme.picture}"/></dt>
									<dd>
										<p class="themeClass">主题：${gameInformationTheme.theme}</p>
										<p>日期：<fmt:formatDate value="${gameInformationTheme.date}" pattern="yyyy-MM-dd" /></p>
										<p class="text-center">
											<a href="gameInformationThemeUpdate?id=${gameInformationTheme.id}" class="updateButton btn btn-info">
												<span class="glyphicon glyphicon-pencil"></span> 修 改
											</a>
											<a href="gameInformationSelect?informationThemeId=${gameInformationTheme.id}" class="updateButton btn btn-info">
												<span class="glyphicon glyphicon-eye-open"></span> 查看内容
											</a>
											<button data-id="${gameInformationTheme.id}" type="button" class="updateButton btn btn-danger delete">
						...
						<div class="text-right buttonMarginSize">
							<a href="gameInformationThemeAdd" class="updateButton btn btn-info">
						...
						<c:if test="${notNull}">
							<div id="pageSizeAndpageNum">
								<ul>
									...
									<c:forEach var="pagingButton" items="${requestScope.gameInformationTheme_pagingButton}">
		...
		<script src="assets/pages/scripts/gameInformationTheme.js"></script>
	</body>
</html>
