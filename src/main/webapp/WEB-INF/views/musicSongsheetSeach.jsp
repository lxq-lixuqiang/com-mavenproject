...
<html>
	<head>
		...
		<link href="assets/pages/css/musicSongsheetSeach.css" type="text/css" rel="stylesheet">
	</head>
	<body>
				...
				<div class="sectionInfo">
					<ul class="borderH3">
						<c:if test="${requestScope.pageInfo.total>0}" var="notNull">
							<c:forEach items="${requestScope.pageInfo.list}" var="musicSongsheet">
									<li>
										<div>
											<a target="_blank" href="musicSongsheetInfoVisit?musicSongsheetId=${musicSongsheet.id}">
												<img src="${pageContext.request.contextPath}/assets/pages/img/musicSongsheetImg/${musicSongsheet.picture}"/>
												<span>
													<label>${musicSongsheet.name}</label>
													<label>日期：<fmt:formatDate value="${musicSongsheet.date}" pattern="yyyy-MM-dd" /></label>
												</span>
											</a>
										</div>
									</li>
		...
		<script id="musicSongsheetSeachTemplate" type="text/template">
			{{#if total}}
				{{#each list}}
					<li>
						<div>
							<a target="_blank" href="musicSongsheetInfoVisit?musicSongsheetId={{id}}">
								<img src="${pageContext.request.contextPath}/assets/pages/img/musicSongsheetImg/{{picture}}"/>
								<span>
									<label>{{name}}</label>
		...
		<script type="text/javascript" src="assets/pages/scripts/musicSongsheetSeach.js"></script>
	</body>
</html>