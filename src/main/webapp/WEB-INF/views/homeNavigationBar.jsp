...
<html lang="zh-CN">
  	<head>
		...
		<link href="assets/pages/css/homeNavigationBar.css" rel="stylesheet">
	</head>
	<body>
			...
			<div class="section">
				...
				<div class="section-botton">
					<div class="section-botton-left">
						...
						<script> 
							$(".sortable-accordion div:nth-of-type(3)").attr("id","showDiv");
					...
					<div class="section-botton-right">
						...
						<div id="currentLocation">
							<p><a href="dataManagement">首页数据</a><span>&#155;</span>导航栏信息</p>
						</div>
						<input type="hidden" id="msg" value="${requestScope.msg}" />
						
						<div>
							<p class="findClass">
								<input type="text" autocomplete="off" id="title" value="${sessionScope.homeNavigationBar_title}" placeholder="搜索名称" maxlength="10" /><img id="select" alt="搜索" src="${pageContext.request.contextPath}/assets/pages/img/common/ImageSelect01.png" />
							</p>
						</div>
						
						<c:if test="${requestScope.pageInfo.total >0}" var="notNull">
							<c:forEach var="homeNavigationBar" items="${requestScope.pageInfo.list}">
								<dl class="dlClass text-left">
									<dt class="text-center"><img class="imgClass" src="${pageContext.request.contextPath}/assets/pages/img/homeNavigationBarImg/${homeNavigationBar.picture}"/></dt>
									<dd>
										<p class="themeClass">名称：${homeNavigationBar.title}</p>
										<p>日期：<fmt:formatDate value="${homeNavigationBar.date}" pattern="yyyy-MM-dd" /></p>
										<p class="text-right">
											<a href="homeNavigationBarUpdate?id=${homeNavigationBar.id}" class="updateButton btn btn-info">
												<span class="glyphicon glyphicon-pencil"></span> 修 改
											</a>
											<button data-id="${homeNavigationBar.id}" type="button" class="updateButton btn btn-danger delete">
						...
						<div class="text-right buttonMarginSize">
							<a href="homeNavigationBarAdd" class="updateButton btn btn-info">
								<span class="glyphicon glyphicon-plus"></span> 添加导航栏信息
							</a>
						</div>
						...
						<c:if test="${notNull}">
							<div id="pageSizeAndpageNum">
								<ul>
									...
									<c:forEach var="pagingButton" items="${requestScope.homeNavigationBar_pagingButton}">
		...
		<script src="assets/pages/scripts/homeNavigationBar.js"></script>
	</body>
</html>
