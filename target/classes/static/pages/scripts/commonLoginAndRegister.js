var CommonLoginAndRegister=function(){
	var numArray=[];
	...
	var saveButton=function(){
		$(".save").click(function(){
			...
			if(ImgPath != null && ImgPath.val().length >0){
				var id=ImgPath.parent().prev().val();
				var formData=new FormData();
				formData.append("file",ImgPath[0].files[0]);
				formData.append("id",id);
				$.ajax({
					async:false,
					url:"commonLoginAndRegisterFileUpload",
					type: "post" ,
					data: formData ,
					processData: false ,
					contentType: false ,
	...
	var fileUpload=function(){
		$(".fileUpload").on('change', function () {
			var ImgPath=$(this);
			var num=ImgPath.prev().data("imgnum");
			var isOk=false;
			var context;
