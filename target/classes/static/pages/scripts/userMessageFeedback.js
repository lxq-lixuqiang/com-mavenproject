var UserMessageFeedback=function(){
	var headInit=function(){
		$("#userMessageFeedback>a").attr("href","javascript:;");
		$(".sectionTopMover>li").mouseenter(function(){
			$(this).attr("class","mover");
			$("#userMessageFeedback").attr("class","click");
		}).mouseleave(function(){
			$(this).attr("class","noClick");
			$("#userMessageFeedback").attr("class","click");
		});
		$("#userHeaderImg").attr("href","javascript:;");
		
		$("#select").mouseenter(function(){
			$(this).attr("src","assets/pages/img/common/ImageSelect02.png");
		}).mouseleave(function(){
			$(this).attr("src","assets/pages/img/common/ImageSelect01.png");
		});
		
		if($("#msg").val().length>0){
			new NoticeJs({
			    text: $("#msg").val(),
			    position: 'middleCenter',
			    animation: {
			        open: 'animated zoomIn',
			        close: 'animated zoomOut'
			    }
			}).show();
		}
	}
	
	var pagingButton=function(){
		$(".ulNotClick").click(function(){
			var pageNum=$(this).data("pagenum");
			location.href="userMessageFeedback?pageNum="+pageNum;
		});
	}
	
	var deleteButton=function(){
		$(".delete").click(function(){
			var id=$(this).data("id");
			Lobibox.confirm({
				title: '温馨提示：',
			    msg: "确定删除吗？",
			    width: 300,
			    buttons: {
			    	yes: {
			    	    'class': 'lobibox-btn lobibox-btn-no',
			    	    text: '确 定',
			    	    closeOnClick: true
			    	  },
			    	  no: {
			    	    'class': 'lobibox-btn lobibox-btn-default',
			    	    text: '取 消',
			    	    closeOnClick: true
			    	  }
			    },
			    callback: function ($this, type, ev) {
			        if(type == 'yes'){
			    		location.href="deleteUserMessageFeed?id="+id;
			        }
			    }
			});
		});
	}
	
	function seach(){
		var type=$("#type").val();
		var title=$("#title").val();
		location.href="userMessageFeedback?type="+type+"&title="+title+"&pageNum=1";
	}
	var seachButton=function(){
		$("#select").click(function(){
			seach();
		});
	}
	var seachKeyUp=function(){
		$("#title").keyup(function(e){
			if(e.which == 13){
				seach();
			}
		});
	}
	
	return {
		init:function(){
			headInit();
			pagingButton();
			deleteButton();
			seachButton();
			seachKeyUp();
		}
	}
}();
$(function(){
	UserMessageFeedback.init();
});